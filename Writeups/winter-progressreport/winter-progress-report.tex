...
At the start of January, we noticed that the computed eclipse
times from the Ephemeris JavaScript library were a bit off from what other
sources stated. Locations that were within the path of totality were merely
seconds off the actual eclipse time, but as we deviated from the path of totality, 
the computed times were up to 20 minutes off. We
looked into several potential solutions to give more accurate results. One 
of the options was to perform an interpolation with known data points from
NASA to to compute eclipse times anywhere in the country. We implemented a
test script in Python to test if this idea was realistic and the results
reflected that it was. While looking into converting this Python interpolation
to JavaScript, we discovered another library called MeeusJs. We found that 
the eclipse times that this library computed were much more accurate than 
Ephemeris. This prevented us from wasting time and energy to implement
the interpolation in JavaScript.
